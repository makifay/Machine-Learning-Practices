% BLG527E, HW2, Q1, Linear Discriminant Analysis
...
display('')
display('#####  Linear Discriminant Analysis  #####')
...
% >> Training data is used for training and validation
  ...
  false_prediction = lda_main(Xt_L0, Xt_L1, Xt_L0, Xt_L1, 0 );
...
% >> Training data is used for training and validation data is used for validation
  ...
  false_prediction2 = lda_main(Xt_L0, Xt_L1, Xv_L0, Xv_L1, 0 );
...
%display('=== Results for plotting of Validation data decision boundary:');
 lda_main(Xt_L0, Xt_L1, Xv_L0, Xv_L1, 1 );  % plot with validation data 
%display('=== Results for plotting of Training data decision boundary:');
 lda_main(Xt_L0, Xt_L1, Xt_L0, Xt_L1, 1 ); % plot with training data 
...
function false_pred = lda_main(Xt_L0, Xt_L1, Xv_L0, Xv_L1, plotSelection )
...
% summation of covariances to attain the shared covariance matrix
sigma_shared = sigma_L0 + sigma_L1 ;

% >> Finding of the  b, w parameters 

  b = (mu_L0*inv(sigma_shared)*mu_L0' - mu_L1*inv(sigma_shared)*mu_L1')/2 + ... 
log(p_Y/(1-p_Y));

  w = inv(sigma_shared) * (mu_L1 - mu_L0)';

% >> Classification Errors
  prediction_label = 1./(1+exp(-w(1)*Xv_data(:,1)-w(2)*Xv_data(:,2)-b));

  prediction_L0 = 1./(1+exp(-w(1)*Xv_L0(:,1)-w(2)*Xv_L0(:,2)-b));

  prediction_L1 = 1./(1+exp(-w(1)*Xv_L1(:,1) - w(2)*Xv_L1(:,2)-b));
...
%disp(['False Prediction of Label-0 : ' num2str(falsePrediction_L0) ' (over ' num2str(size(Xv_L0,1)) ' Label-0 data)'  ]);
  ...
  if(plotSelection == 1)
	% Firstly creating data for contour, then calculate linear discriminant values 
	x_sample = [-6:.01:6];
	y_sample= [-6:.01:10];
	[x_mg ,y_mg] = meshgrid(x_sample, y_sample);

        linear_disc_vals = 1./(1+exp(-w(1)*x_mg-w(2)*y_mg-b));
	...
	title( 'Decision Boundary of Linear Discriminant Classifier ' );
        legend('label=0', 'label=1','False Predictions');
	xlabel('x');
	ylabel('y');
	[Cntr,con_h] = contour(x_mg, y_mg, linear_disc_vals, [p_Y ,p_Y], 'm');
